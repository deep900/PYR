...
public class PyrMessenger implements Messenger{
	...
	public boolean sendEmailMessage(EmailMessageObject emailMessageObject) {
		...
		if (mimeMessage == null) {
			mimeMessage = establishSecondaryEmailConnection();
		}
		try {
	...
	private Properties getPrimaryProperties() {
		...
		Resource file = new ClassPathResource("email_settings_primary.properties");
		InputStream inputStream = null;
		try {
			inputStream = file.getInputStream();
			prop.load(inputStream);
			System.out.println("Loaded the properties for email..");
	...
	private MimeMessage establishPrimaryEmailConnection() {
		try {
			...
			System.out.println("Printing the properties " + prop);
			email_user_primary = prop.getProperty("username").toString();
			email_password_primary = prop.getProperty("password").toString();
			Session session = Session.getInstance(getPrimaryProperties(), new javax.mail.Authenticator() {
	...
	private MimeMessage establishSecondaryEmailConnection() {
		return null;
	}
