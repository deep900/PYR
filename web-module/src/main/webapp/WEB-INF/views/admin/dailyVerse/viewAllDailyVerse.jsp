					...
					<section class="grid">
						<div class="heading-holder">
							<h3 class="content-heading">
								<span><em class="h-left"></em><span class="inner-heading">View Daily Verse Entry</span><em class="h-right"></em></span>
							</h3>
						</div>
						
							<div class="blog-post-admin">								
								<table style="margin-botton:10px;text-align:center;border-collapse:collapse;border-spacing:0;margin-left:35px;">
								<thead>
								<tr>
								<th>ID</th>
								<th>Eng Chapter</th>
								...
								<tbody>
								<c:forEach items="${dailyBibleVerseList}" var="dailyVerse">
										<tr>										
										<td><c:out value="${dailyVerse.id}"></c:out></td>											
										<td><c:out value="${dailyVerse.engChapter}"></c:out></td>
                                        <td><c:out value="${dailyVerse.engVerse}"></c:out></td>
										<td><font face="CustomTamil50"><c:out value="${dailyVerse.chapter}"></c:out></font></td>
										<td><font face="CustomTamil50"><c:out value="${dailyVerse.verse}"></c:out></font></td>
										<td><a href="<c:out value='${context}/admin/editBibleVerse?id=${dailyVerse.id}&key=${token}'></c:out>">View/Edit</a></td>
										<td><a href="#" onclick=showWarning('<c:out value='${context}/admin/deleteDailyVerseEntry?id=${dailyVerse.id}&key=${token}'></c:out>') >Delete</a></td>
										</tr>
									</c:forEach>
								</tbody>
								</table>								
							...
							<hr>
							<strong class="short-heading1">More Daily Verses<span class="h-line"></span></strong>
								<ul class="pagination-list color">
									<li><a href="<c:url value="viewAllDailyVerses?key=${token}&startIndex=${currentIndex}&action=previous"/>" class="prev">previous</a></li>
									<c:set var="count" value="0" scope="page" />
									<c:forEach begin="1" end="${maxRecordsCount}" step="${maxRecordsPerPage}" varStatus="loop">
									<c:set var="count" value="${count + 1}" scope="page"/>
									<li><a href="<c:url value="viewAllDailyVerses?key=${token}&startIndex=${count}&action=nextSet"/>">${count}</a></li>
									</c:forEach>
									<li><a href="<c:url value="viewAllDailyVerses?key=${token}&startIndex=${currentIndex}&action=next"/>" class="next">Next</a></li>
								</ul>
							</div>					
