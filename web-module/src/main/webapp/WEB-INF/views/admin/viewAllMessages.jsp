					...
					<section class="grid">
						<div class="heading-holder">
							<h3 class="content-heading">
								<span><em class="h-left"></em><span class="inner-heading">View Messages</span><em class="h-right"></em></span>
							</h3>
						</div>
							...
							<div class="blog-post-admin">
								...
								<c:forEach items="${objectList}" var="msgModel">
										<tr>										
										<td><c:out value="${msgModel.id}"></c:out></td>
										<td><c:out value="${msgModel.messageTitleEnglish}"></c:out></td>												
										<td><font face="CustomTamil50"><c:out value="${msgModel.messageTitleTamil}"></c:out></font></td>
										<td><c:out value="${msgModel.lastModified}"></c:out></td>
										<td><a href="#" onclick=redirect('<c:out value='${context}/admin/editPhoto?id=${msgModel.id}&key=${token}'></c:out>') >Edit</a></td>
										<td><c:out value="${msgModel.getPhotoStatus()}"></c:out></td>
										<td><a href="#" onclick=redirect('<c:out value='${context}/admin/editMessage?id=${msgModel.id}&key=${token}'></c:out>') >View/Edit</a></td>
										<td><a href="#" onclick=showWarning('<c:out value='${context}/admin/deleteMessage?id=${msgModel.id}&key=${token}'></c:out>') >Delete</a></td>
										</tr>
									</c:forEach>
								</tbody>
								</table>	
