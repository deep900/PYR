					...
					<section class="grid">
						<div class="heading-holder">
							<h3 class="content-heading">
								<span><em class="h-left"></em><span class="inner-heading">View All Bible Quiz</span><em class="h-right"></em></span>
							</h3>
						</div>
							...
							<div class="blog-post-admin">
								...
								<tbody>
								<c:forEach items="${englishQuizList}" var="bqModel">
										<tr>										
										<td><c:out value="${bqModel.id}"></c:out></td>											
										<td><c:out value="${bqModel.level}"></c:out></td>
										<td><c:out value="${bqModel.testament}"></c:out></td>
										<td><c:out value="${bqModel.question}"></c:out></td>										
										<td><c:out value="${bqModel.correctAnswer}"></c:out></td>
										<td><c:out value="${bqModel.bibleReference}"></c:out></td>
										<td><a href="#" onclick=showWarning('<c:out value='${context}/admin/deleteBibleQuiz?id=${bqModel.id}&key=${token}&lang=${langauge}'></c:out>') >Delete</a></td>
										</tr>
									</c:forEach>
								</tbody>
								</table>								
							</div>		
							<p style="margin-left:68px"> Tamil Questions </p><br>
							<div class="blog-post-admin">
								<br class="clear" />
								<table style="width:auto;margin-botton:10px;text-align:center;border-collapse:collapse;border-spacing:0;margin-left:35px;">
								<thead>
								...
								<tbody>
								<c:forEach items="${tamilQuizList}" var="bqModel">
										<tr>										
										<td><c:out value="${bqModel.id}"></c:out></td>											
										<td><c:out value="${bqModel.level}"></c:out></td>
										<td><c:out value="${bqModel.testament}"></c:out></td>
										<td><font face="CustomTamil50"><c:out value="${bqModel.question}"></c:out></font></td>										
										<td><font face="CustomTamil50"><c:out value="${bqModel.correctAnswer}"></c:out></font></td>
										<td><font face="CustomTamil50"><c:out value="${bqModel.bibleReference}"></c:out></font></td>
										<td><a href="#" onclick=showWarning('<c:out value='${context}/admin/deleteBibleQuiz?id=${bqModel.id}&key=${token}&lang=${langauge}'></c:out>') >Delete</a></td>
										</tr>
									</c:forEach>
								</tbody>
