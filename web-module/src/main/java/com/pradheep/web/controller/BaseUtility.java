...
public class BaseUtility<T> {
	...
	private String _appKey_web = "TEv2NsXNPjs=";
	...
	public String _issue_PrivateToken() {
		...
		return PublicUtility.getInstance(_appKey_web).EncryptText(buffer_.toString());
	}
	...
	public boolean isTokenValid(String token) {
		...
		token = token.replace(" ", "+");
		PublicUtility utility = PublicUtility.getInstance(_appKey_web);
		String parse = utility.DecryptText(token);
